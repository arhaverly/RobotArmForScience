...
from utils.sensitives import ip_crest_backend, mp_api
from utils.utils import email, post_to_ot2, post_to_rt
...
def flush_memory():
    ...
    resp = requests.get(f'{ip_crest_backend}/flush')
    return f"{resp.text}"
...
def load_user_profile(user_name: str):
    ...
    if user_name not in ['chu', 'zhen', 'ju']:
...
def run_opentrons(
    ...
    resp = post_to_ot2('update_tasks', tasks_dict)
...
def email_video(user_name: str, content: str):
    ...
    resp = post_to_ot2('email_video', data)
...
def phase_query(elements: str):
    ...
    with MPRester(api_key=mp_api) as mpr:
        try:
            ret_val = mpr.thermo.get_phase_diagram_from_chemsys(elements, thermo_type="GGA_GGA+U")
        except Exception:
...
def pump_control(exp_name: str, action: str):
    ...
    resp = post_to_rt('pump_control', data)
...
def gas_control(exp_name: str, action: str):
    ...
    resp = post_to_rt('gas_control', data)
...
def laser_cut(exp_name: str):
    ...
    resp = post_to_rt('laser_cut', data)
