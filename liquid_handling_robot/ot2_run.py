...
class OT2_Config:
    ...
    def pipette_base(
        ...
        if target_type == 'sample':
            ...
            pipettor.dispense(volume, rate=self.config['pipetting_rate_to_sample'])
            ...
            pipettor.dispense(volume, rate=self.config['pipetting_rate_to_mixing_well'])
    ...
    def pipette(self, source, d_to_bottom, volume, target_type, target, change_tip=True):
        ...
        if len(self.tipracks['20ul']) == 0 or len(self.tipracks['300ul']) == 0:
            ...
            for i in range(int(div)):
                ...
                self.pipette_base(tip_type, source, d_to_bottom, v, target_type, target, change_tip)
            # if pipetting vol is 0, ot2 will pipette maximum capacity of current tip by default, which is undesired
            if mod > 0:
                self.pipette_base(tip_type, source, d_to_bottom, mod, target_type, target, change_tip)
        ...
        # two types pipette available case, auto-switch
            ...
            # if pipetting vol is 0, ot2 will pipette maximum capacity of current tip by default, which is undesired
                ...
                for i in range(int(div_2)):
                    ...
                    self.pipette_base(tip_type, source, d_to_bottom, v, target_type, target, change_tip)
                if mod_2 > 0:
                    self.pipette_base(tip_type, source, d_to_bottom, mod_2, target_type, target, change_tip)
