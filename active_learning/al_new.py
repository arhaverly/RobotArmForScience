...
from ax import (
    ...
    OptimizationConfig,
    Runner, SumConstraint, ParameterConstraint
)
...
class AL:
    def __init__(self, exp_name, load_from_db=True, connect_ot2=True, ot2_env='linux', user='li'):
        ...
        self.db = Database(exp_name=self.exp_name, user=user)
        self.bo_model = None
        self.gs = None

        # exp setup
        self.elements = self.al_configs['elements']
        # self.battery = self.al_configs['battery']
        ...
        parameter_constraint1 = ParameterConstraint(constraint_dict=constraint_dict3, bound=bound3)
        parameter_constraint2 = ParameterConstraint(constraint_dict=constraint_dict4, bound=bound4)
        ...
        if load_from_db:
            ...
            self.search_space = SearchSpace(
                parameters=[
                    RangeParameter(
                                    name='u', parameter_type=ParameterType.FLOAT, lower=0.0, upper=1.0, digits=3
                                ),
                    RangeParameter(
                                    name='v', parameter_type=ParameterType.FLOAT, lower=0.0, upper=2.0, digits=3
                                ),
                    RangeParameter(
                                    name='x', parameter_type=ParameterType.FLOAT, lower=0.0, upper=1.0, digits=3
                                ),
                    #RangeParameter(
                                   # name='y', parameter_type=ParameterType.FLOAT, lower=0.0, upper=1.0, digits=3
                   # ),
                    # RangeParameter(
                ...
                # add constraints
                parameter_constraints=[parameter_constraint1, parameter_constraint2]
    ...
    def convert_trial_to_df(self, trial, add_pred=True, model=None):
        ...
        for arm in trial.arms:
            data_dict = {
                ...
                # self.elements[i]: self.elements[i] for i in range(len(self.elements) - 1),
    ...
    def add_manual_trial(self, trial_df: pd.DataFrame):
        ...
        arm_list = []
        for i, arm in trial_df.iterrows():
            arm_list.append(Arm({self.elements[i]: arm[self.elements[i]] for i in range(len(self.elements) )}))
