...
(iii) PRE-IMAGING STATE: double check if the image contains ROI and the ROI is correctly centered. If not, go back to SEARCHING STATE. If yes, proceed to IMAGING STATE.
(iv) IMAGING STATE: is the ROI precisely centered? If no, then estimate the coords of the ROI again and recenter it. If yes, then check if the horizontal field width correctly set as final objective? if yes, take a picture of the ROI, if no, set FW accordingly and take a picture of the ROI.
